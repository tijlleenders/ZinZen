...
test("test", async ({ browser }) => {
  ...
  await userOnePage.getByRole("button", { name: "My Goals Goals" }).click();
  ...
  await userOnePage
    ...
    .first()
    .click();
  await userOnePage.getByRole("button", { name: "share with friend Share privately" }).click();
  ...
  await userTwoPage.getByRole("button", { name: "English" }).click();
  await userTwoPage.getByRole("button", { name: "Continue zinzen faq" }).click();
  ...
  await userTwoPage.getByRole("button", { name: "My Time" }).click(); // Switch to My Time
  await userTwoPage.getByRole("button", { name: "My Goals" }).click(); // Switch back to My Goals
  await userTwoPage.reload(); // Refresh the window
